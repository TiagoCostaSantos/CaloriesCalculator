...
import com.CaloriesCalculator.facade.HomeFacade;
...
public class HomeController {
    ...
    public HomeController(UsuarioUseCase usuarioUseCase, CookieUseCase cookieUseCase, CalculosUseCase calculosUseCase, FichaAlimentarUseCase fichaAlimentarUseCase, ProdutoAlimenticioUseCase produtoAlimenticioUseCase, HomeFacade homeFacade){
...
//        model.addAttribute("outroConsumida", outroConsumida);
        ...
        HomeViewModel vm = homeFacade.montarHome(principal, request, false, model);


        // se já não existir cria um
        if (!model.containsAttribute("usuario")) {
            model.addAttribute("usuario", new UsuarioModel());
        }

        model.addAllAttributes(vm.toMap());
        return "index";
    }
